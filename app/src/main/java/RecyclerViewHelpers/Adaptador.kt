 ...
 class Adaptador(private var Datos: List<Ticket>) : RecyclerView.Adapter<RecyclerViewHelper.ViewHolder>(){
     ...
     override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerViewHelper.ViewHolder {
         ...
         val vista =
             LayoutInflater.from(parent.context).inflate(
                 rafa.vargas.myapplication.R.layout.activity_itemcard,
                 parent,
                 false
             )
     ...
     fun actualizarDato(
         ...
         GlobalScope.launch(Dispatchers.IO) {
             ...
             val añadirTicket =
                 objConexion?.prepareStatement("UPDATE ticket SET Titulo = ?, Descripcion = ?, Autor = ?, AutorEmail = ?, TicketStatus = ?,FinishDate = ?  WHERE UUID_Tickets = ?")!!
             añadirTicket.setString(1, Titulo)
             añadirTicket.setString(2, Descripcion)
             añadirTicket.setString(3, Autor)
             añadirTicket.setString(4, Autoremail)
             añadirTicket.setString(5, TicketStatus)
             añadirTicket.setString(6, Finishdate)
             añadirTicket.setString(7, UUID_Tickets)

             añadirTicket.executeUpdate()
